...
class Module(ABC):
    """
    Abstract base class that all Kai modules must implement.
    ...
    This is like a contract: "If you want to be a Kai module,
...
class KaiEngine:
    ...
    ----------------------------------------------
    Notice how KaiEngine doesn't inherit from Module — it CONTAINS modules.
    ...
    async def start(self) -> None:
        ...
        """
        logger.info("Kai Engine starting...")
        self._running = True
        ...
        logger.info("Kai is ready. Listening for input...")
    ...
    async def _discover_and_register_modules(self) -> None:
        ...
        module_loaders = [
            ("Voice Listener", "kai.voice.listener", "VoiceListener"),
            ("Speech Recognizer", "kai.speech.recognizer", "SpeechRecognizer"),
            ("Speech Synthesizer", "kai.speech.synthesizer", "SpeechSynthesizer"),
            ("Intent Classifier", "kai.speech.intent", "IntentClassifier"),
            ("Camera Stream", "kai.vision.camera", "CameraStream"),
            ("Motion Detector", "kai.vision.motion", "MotionDetector"),
            ("Gesture Recognizer", "kai.vision.gesture", "GestureRecognizer"),
            ("File Manager", "kai.io.file_manager", "FileManager"),
        ]
    ...
    async def _main_loop(self) -> None:
        """
        The main event loop that keeps Kai alive.
