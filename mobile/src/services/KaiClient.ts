/**
 * KaiClient — HTTP + WebSocket client for communicating with the Kai server.
 */

export interface KaiStatus {
  status: string
...
type EventCallback = (event: KaiEvent) => void
...
export class KaiClient {
  ...
  async getStatus(): Promise<KaiStatus> {
  ...
  connectWs(): void {
    ...
    this.ws.onopen = () => {
      console.log('[KaiClient] WebSocket connected')
    }

    this.ws.onmessage = (event) => {
      try {
        const parsed: KaiEvent = JSON.parse(event.data)
    ...
    this.ws.onclose = () => {
      console.log('[KaiClient] WebSocket disconnected')
      this.ws = null
